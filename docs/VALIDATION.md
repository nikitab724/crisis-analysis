...
The real backend follow-up mapped “flood in san mateo ca” in **2.91 seconds** and rejected a disaster-movie post in **2.00 seconds**. The backend logged a retry after another actual Jev 503 during these checks, and both requests completed successfully. Saved backlog processing remains suspended and the queued records are preserved. Persistent provider outages can still fail explicitly after the bounded retry.
...
- All **175 regression tests passed**, including the new sentence boundary, source timestamp, bounded/malformed context, candidate filter, batch screen, and retry tests. Targeted lint and whitespace checks passed. Tests were run with `CRISIS_CLASSIFICATION_MODE=rules CRISIS_RELEVANCE_MODE=off JEV_BATCH_SCREEN=off` to isolate the original baseline from the operator's live `.env`; semantic-mode tests explicitly enable their test configuration.
- Seven real NLP/gazetteer/Jev example checks passed after one provider failure interrupted the first attempt. Portland without a clue remained unplotted; Willamette River selected Portland, Oregon; Casco Bay selected Portland, Maine. The supplied New Mexico text mapped as a state-level Flood. Explicit Solon/Iowa drowning and Mora/New Mexico outage examples received the corresponding new categories. A canyon merely described as “prone to flood” stayed unplotted. These are bounded smoke tests, not overall accuracy measurements.
- The original supplied public posts were separately tested with their actual parent/headline metadata. New Mexico mapped as Flood. The lake report classified as Drowning and the electricity report as Power Outage, but both remained unmapped for lack of a supported location. No profile-location or school-city guess was added. These limitations remain visible in the classification documentation.
- After the owner explicitly approved live activation, enabled `CRISIS_CLASSIFICATION_MODE=jev` with expanded candidate screening, bounded parent/root and link-card context, and the previously authorized uncapped setting. Existing report CSVs and durable queued work were preserved. The repository and offline fixture still default to the original rules.
- The sample-data dashboard's actual map callback contains five points: the two Portlands, a New Mexico state marker, Solon, and Mora. The underlying examples are labeled separately from live reports. Local Gunicorn hit the previously observed NumPy startup error; the sample preview runs with the already-installed Waitress server instead. Map callback checks establish coordinates and data, not browser screenshot verification.
- Intermittent provider failures still occur. Transient failures now back off 2–30 seconds rather than pausing 30 seconds after every first failure. Rate/auth/quota responses retain a minimum 30-second delay; `Retry-After` is honored. Two concurrent calls and durable retries remain enforced. Diagnostics expose only a sanitized failure kind, not credentials or raw provider error bodies.
- Broader candidates initially created more transformer work and the provider returned HTTP 503 responses. Added an optional batched event screen before NLP/context fetching: at most 16 questions and 12,000 characters per request, retaining uncertain scores of 0.2 or higher. The real ten-example screen kept all eight plausible emergency examples and excluded two clear negatives in one request. Enabled this screen on the approved live mode. This is a throughput aid with a documented recall tradeoff; provider outages can still create a backlog.
