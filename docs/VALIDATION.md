...
Cloud provisioning remains pending: no Render API credential is connected, and browser access to the Render dashboard was blocked because the browser tool could not verify its admin-enforced security policy. The README includes the branch-specific Deploy to Render link and settings. No public deployment URL is claimed until Render reports a successful deployment.
