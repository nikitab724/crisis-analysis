...
- The live dashboard and totals cover the last 24 hours. Expired reports are removed from active CSVs; manually saved archives are separate. Restarts can restore unexpired records with `--resume-from`; see [backend startup](BACKEND.md). This setup does not make the model run on Render or provide independent, always-on processing.
