...
Live mode uses `CRISIS_PROCESSING_WORKERS=4` (valid range 1–4) and `CRISIS_BATCH_SIZE=100` (1–100). An entire available batch goes through the existing conservative token-rule gate in one private `/disaster_candidates` request. Only explicit negative decisions skip further work; malformed responses retain the batch, and an older server without that endpoint falls back to full per-post extraction. The notebook's rules and cleaning are unchanged.

Post workers overlap network waits for the model service, gazetteer, and Jev. The model server uses five HTTP threads, including capacity for readiness/screening, while a lock protects the single loaded transformer. Supabase queries use their existing filters, timeout, and HTTPX connection pool. No extra model copies or paid host are created. CSV writes, progress updates, output ordering, and queue acknowledgement stay on the processor's main thread.

Jev allows at most two requests in flight. Those requests share one atomic 200-call default budget, one cooldown, and a 512-entry cache. Simultaneous identical requests share a decision. Requests waiting for a slot recheck the budget/cooldown before sending. Classification prompts and confidence thresholds are unchanged. This increases concurrency, not the allowed number of API calls. `JEV_MAX_CALLS_PER_RUN` still stops further calls when exhausted; the queue remains intact and the dashboard reports the configured limit. `/activity` exposes `jev_calls`, `jev_max_calls`, and `jev_cooldown_seconds` for diagnosis without adding routine UI counters.

Run `python tests/check_processing_throughput.py` for a repeatable burst comparison using controlled service delays and the real queue/processor code, with no live Jev calls. Its speedup is a test workload measurement, not a guarantee during provider outages or unusually expensive posts.

## Continuous ingestion

The collector keeps its WebSocket open independently of processing. New `app.bsky.feed.post` create operations are decoded by their exact CAR record ID and saved in a SQLite queue. Author DIDs are retained directly instead of blocking collection on handle resolution; Bluesky source links still work. Updates, deletions, likes, and repository snapshots are not classified as new posts.

The processor retrieves up to 100 queued posts per request and acknowledges a batch only after its analysis and CSV writes succeed. Failed model/Jev calls, failed writes, or lost acknowledgements leave the batch pending. Successful per-post results are retained in memory for the active receipt, so retries rerun only unfinished posts; a restart can safely repeat the whole pending batch. Pending work survives a collector restart. Source URIs prevent replay duplication; different posts with identical wording remain distinct. Saved post/location pairs are deduplicated on retry, and counts are rebuilt from the complete saved records to recover after a crash between the two CSV writes. The pair is still not a single atomic transaction, and activity counters are not an exactly-once ledger.

Queue files and the stream cursor live in ignored **`.demo-live/ingest/`**, independently of the temporary report directory. Override this with an absolute `CRISIS_INGEST_DIR` on persistent local storage. Run only one collector and one processor per queue/output pair; the collector uses an OS file lock to reject a second owner. Keep the directory when restarting; deleting it discards pending work and starts collection at the current stream position. Report history still needs `--resume-from`, as described above. A hosted filesystem without a persistent disk cannot preserve this queue across redeploys.

The saved cursor advances in the same SQLite transaction as incoming posts. Normal reconnects resume from it. If the queue reaches **100,000 pending posts**, collection stops advancing its cursor and retries after the consumer drains work; it does not silently evict posts. A prolonged outage or backlog can exceed the provider's available replay window. `OutdatedCursor` and oversized commits produce persistent coverage warnings. Invalid frames/records interrupt the stream and retry from the last saved position rather than silently skipping the failed event. This is recovery within the [AT Protocol replay mechanism](https://atproto.com/specs/event-stream), not a guarantee of complete global coverage or historical backfill. A fresh queue begins at the live position. Processed-URI replay history is bounded to seven days and 200,000 entries; the cursor also rejects older stream events, and saved report URIs provide further duplicate protection.

The existing Jev cap and failure rules remain unchanged. If classification is unavailable or its per-process request budget is exhausted, the affected batch waits for recovery while collection continues into the queue. This can increase backlog; it never bypasses the relevance check or silently drops that batch. The configured request cap has not been raised.

The dashboard hides routine activity statistics and recovered error totals. It shows one concise warning for a stale processor, a disconnected/full collector, an active processing error, pending work older than 30 seconds, or an unrecoverable coverage gap. `/activity` retains all diagnostic counters plus `collector` details (`captured`, `acknowledged`, `cursor`, `connections`, `queue_depth`, `oldest_pending_seconds`, `gap_events`, and `last_gap`). The dashboard asks the collector for fresh status even while analysis is busy. NLP/Jev check/error counts include retry attempts. `collection_ms` now measures a queue fetch, not a collection window; `processing_ms` is the latest completed batch. The processor drains full batches without an added pause. It waits 0.1 seconds after a partial/empty batch and two seconds after an analysis error; this never stops the stream. Progress writes are coalesced to at most four per second, plus completion and errors. The dashboard refreshes every two seconds.
...
```sh
python tests/check_continuous_ingestion.py
```

Before transformer inference, the model service runs the saved EntityRuler's exact surface-token matcher using the same text cleaning and tokenizer. Non-matching posts return `skipped_non_crisis=true`, empty entities, and neutral placeholder sentiment; the processor already discards those posts. Actual candidates keep the full original NLP and subsequent checks. The shortcut disables itself for a trained `DISASTER` NER label, annotation-dependent rules, or unknown pipeline components. The original rules' limitations (for example, some inflected forms or punctuation) remain. Repeat the real-model comparison with `python tests/check_disaster_prefilter.py`; its authored probes are not a live accuracy benchmark.
