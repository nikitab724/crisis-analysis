...
The activity row reports posts received, successfully analyzed posts, analysis errors, and the latest processor update. Counts exclude the synthetic startup post; the charts include its extracted records. A batch with no matching crisis posts still advances activity. `/activity` exposes the same counters without credentials or post contents, plus `rule_skipped` (non-candidates rejected before heavy NLP), `collection_ms`, and `processing_ms` (latest completed stage timings). The collector requests up to 20 posts per batch, returns partial batches after two seconds, then reconnects after processing and a 0.1-second pause. The dashboard refreshes every two seconds. Posts published between collection windows can still be missed.
