...
Jev allows at most two requests in flight. Those requests share one atomic request counter, one cooldown, and a 512-entry cache. Simultaneous identical requests share a decision. Requests waiting for a slot recheck the configured budget and cooldown before sending. `JEV_MAX_CALLS_PER_RUN` defaults to 200; **set it to `0` and restart the processor to disable the app's cap**. A positive limit stops further calls when exhausted, keeps the queue intact, and produces a limit warning. Provider failures still retain pending work and apply the 30-second cooldown in uncapped mode. Classification prompts and confidence thresholds are unchanged. `/activity` exposes `jev_calls`, `jev_max_calls` (`0` means uncapped), and `jev_cooldown_seconds` for diagnosis without adding routine UI counters.
