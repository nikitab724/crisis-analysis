...
- Bounds each request to 32 candidate pairs, 10,000 text characters, and connect/read timeouts of 3/6 seconds. Successful evaluations are cached in memory (512 entries). Requests have no automatic retries; transient failures back off for 2, 4, 8, 16, then 30 seconds. Authentication, quota, and rate-limit responses retain a minimum 30-second delay, and provider `Retry-After` headers are honored. A successful response resets the transient failure streak. Requests remain bounded to two in flight; the queue retries unsuccessful work.
- Request starts are spaced across all workers by `JEV_MIN_REQUEST_INTERVAL` (default 1 second, configurable from 0–30). HTTP 429/503 doubles that spacing up to 30 seconds; 20 consecutive successful calls reduce it gradually toward the configured minimum. The processor leaves failed batches queued during the provider cooldown instead of repeating context and transformer work. This reduces bursts; it cannot increase [the provider's capacity or Gateway's free-tier limits](https://vercel.com/docs/ai-gateway/rate-limits). If incoming work exceeds available throughput, the queue still grows.
- Optional batch-screen decisions are also cached per exact post/headline text (2,048 entries). A retry containing fewer pending posts reuses their successful decisions without sending the same screening questions again. Failures are never cached as negative decisions; labels, thresholds, and location evidence requirements are unchanged.
- `JEV_MAX_CALLS_PER_RUN` defaults to 200 attempts per processor process, **shared by location selection, relevance screening, and optional crisis classification**, including failed attempts. Set it to **`0` in the backend's `.env` and restart the processor to disable the app's cap**. Positive integers retain a finite allowance; negative values are rejected. Two concurrent requests, caching, timeouts, and the shared failure backoff apply in both modes. `/activity` reports `jev_max_calls: 0` when uncapped and continues counting requests. A restart resets the count; this is not a billing limit. Provider budgets, credits, and rate limits remain authoritative. Check [current Gateway pricing](https://vercel.com/ai-gateway/models/jev); this code never purchases credits or enables top-ups.
