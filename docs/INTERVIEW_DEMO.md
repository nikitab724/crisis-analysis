...
An identical test can reuse its validated real result for up to 60 seconds in the Render worker (128 entries maximum). Failed requests are never cached; a changed post or expired entry runs real analysis again. The cache is separate from the predefined replay, uses no persistent storage, and is invalidated by a backend URL or credential change. Backend connections are reused within each worker thread. Interactive tests retain the configured minimum request interval and provider cooldowns without inheriting the live collector's escalating batch cadence. Fresh text still needs the Mac, gazetteer, and Jev; this setup does not promise instant first-time predictions.
