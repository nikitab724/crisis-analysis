...
The recovered data contains many places called Austin. The current resolver uses the explicit Texas context and requires a unique city match. An unqualified ambiguous Austin is left unresolved instead of choosing by population. The supporting Texas entity no longer adds another record, so the known post produces one Austin/Texas location. The NLP model is unchanged; see [validation](VALIDATION.md) for earlier checkpoints and the current matching behavior.
